<template>
  <div>
    <div style="padding:2rem; max-width:600px;">
      <h1>Receive Payment - Invoice #{{ invoiceId }}</h1>
      <form @submit.prevent="submit">
        ...
        <router-link :to="`/companies/${id}/invoices/${invoiceId}`"><button type="button" style="margin-left:0.5rem;">Cancel</button></router-link>
      </form>
    </div>
  </div>
</template>
...
const submit = async () => {
  await axios.post(`/companies/${id}/invoices/${invoiceId}/receive`, form.value);
